...
7. 受け入れ基準がルール（`vault/rules/` 配下のファイル名や「コーディングルールに従う」等）を参照している行は、該当ルールファイル（手順5で読んだもの）を根拠に真偽を判定し、`note` に参照したルールファイルのパスを書く。受け入れ基準がルールを参照していない行は、ルールを判定の根拠にしない
8. `vault/verdicts/<計画ID>/<id>.json` を以下の形式で書く（既存があれば上書き）
