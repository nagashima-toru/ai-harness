...
```
複製されるものは `scripts/install.sh` 冒頭のコメントのとおり（`.claude/`（`ai-harness.md` を含む）、`vault/` のテンプレートと標準ルールと空の `todo.md`、`scripts/smoke.sh`、`scripts/rules.sh`、`scripts/merge_claude_md.py`、`scripts/merge_settings_json.py`、`docs/vault-spec.md`、`CLAUDE.md`）。既存ファイルは上書きしない。
